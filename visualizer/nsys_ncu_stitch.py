...
def stitch_nsys_ncu(nsys_trace_file, ncu_raw_file, cols=None):
    ...
    def lookup_kernel(kname, kthreads):
        ...
        for idx in idxs:
            ...
            if best_idx is None or np.abs(nthreads - kthreads) < np.abs(
                best_nthreads - kthreads
            ):
                best_idx = idx
    ...
    print(
        f"WARNING: {len(not_found)}/{len(tot_kernels)} kernels not found in NCU file")
...
if __name__ == "__main__":
    ...
    stitch_nsys_ncu(sys.argv[1], sys.argv[2])
