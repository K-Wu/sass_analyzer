...
class SassLineType(Enum):
    ...
    InsCode = 0, re.compile(
        r"^\s*\/\*(?P<addr>\w+)\*\/\s*\{?\s*(?P<asm>.*;)\s*\/\* (?P<code>.*) \*\/"
    )
    ...
    def getCallbackArgs(cls, op, res):
        ...
        if op == cls.InsCode:
            return (
                int(res.group("addr"), 16),
                res.group("asm"),
                int(res.group("code"), 16),
            )
        elif op == cls.InsOnly:
...
class ParserState(Enum):
    ...
    WaitForArch = (
        auto()
    )  # Deprecated, the arch will be switched inline, thus no need to transfer to this state
...
class ParserStateMachine:
    ...
    def feed(self, op):
        ...
        if op not in self.tr_matrix[self.state]:
            raise Exception(
                f"Invalid input {op} for state {self.state} (Unacceptable op)!!!"
            )
        ...
        if self.state not in self.tr_matrix or self.state == PS.Invalid:
            raise Exception(
                f"Invalid input {op} for state {self.state} (Invalid transferred state)!!!"
            )
...
class CuInsFeeder:
    def __init__(self, fstream, archfilter=None, insfilter=None):
        ...
        self.__TMs = {
            "default": self.__getTrMatrixDefault(),
    ...
    def __feedLineOp(self, op, *args):
        ...
        if op not in self.__CurrTM[self.__mPState]:
            raise Exception(
                f"Invalid input {op} for state {self.__mPState} (Unacceptable op) @Line{self.__mLineNo:04d}!!!"
            )
        ...
        if self.__mPState not in self.__CurrTM or self.__mPState == PS.Invalid:
            raise Exception(
                f"Invalid input {op} for state {self.__mPState} (Invalid transferred state) @Line{self.__mLineNo:04d}!!!"
            )
    ...
    def __iterPopIns(self):
        ...
        for addr, code, asm, ctrl in zip(
            self.__mAddrList, ilist, self.__mAsmList, clist
        ):
            yield addr, code, asm, ctrl
    ...
    def __switchArch(self, arch):
        ...
        if smversion.getMajor() == 3:
            ...
            raise NotImplementedError(
                f"ERROR! No implemented state machine for arch {smversion}!!!"
            )
    ...
    def __getTrMatrixForSM_3x(self):
        ...
        stm.addop_dict(
            PS.Ready,
            {
                ...
                SLT.InsCode: (
                    PS.Ready,
                    self.__pushInsCode_3x,
                ),  # sometimes the padded ins may just follow normal ins
                # no extra ctrl line.
                SLT.Others: (PS.Ready, None),
            },
        )
        ...
        stm.addop_dict(
            PS.Ready,
            {
                ...
                SLT.InsCode: (
                    PS.Ready,
                    None,
                ),  # sometimes the padded ins may just follow normal ins, no ctrl line.
                SLT.Others: (PS.Ready, None),
            },
        )
    ...
    def __getTrMatrixForSM_7x8x(self):
        ...
        stm.addop_dict(
            PS.Ready,
            {
                ...
                SLT.CodeOnly: (
                    PS.Ready,
                    lambda x: self.__emitMessage("Missing Instruction"),
                ),
                SLT.Others: (PS.Ready, None),
            },
        )
...
if __name__ == "__main__":
    feeder = CuInsFeeder("sass_analyzer/test/export_op.dump.sass")
    feeder.extract("sass_analyzer/test/export_op.dump.sass.out")
