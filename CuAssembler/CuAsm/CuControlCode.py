...
c_ControlCodesPattern = re.compile(
...
class CuControlCode:
    def __init__(self, code):
        c_waitbar, c_readbar, c_writebar, c_yield, c_stall = CuControlCode.splitCode(
            code
        )
    ...
    def decode(code):
        ...
        s_waitbar = "".join(
            ["-" if (c_waitbar & (2**i)) == 0 else "%d" % i for i in range(6)]
        )
        s_stall = "%02d" % c_stall

        return "B%s:R%s:W%s:%s:S%s" % (
            s_waitbar,
            s_readbar,
            s_writebar,
            s_yield,
    ...
    def encode(s):
        ...
        code = CuControlCode.mergeCode(
            c_waitbar, c_readbar, c_writebar, c_yield, c_stall
        )
