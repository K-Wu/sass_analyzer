...
class CuSMVersion(object):
    ...
    PadBytes_5x_6x = bytes.fromhex(
        "e00700fc00801f00 000f07000000b050 000f07000000b050 000f07000000b050"
    )
    ...
    FloatImmeFormat_5x_6x = {
        "H": ("e", "H", 16, 16),
        "F": ("f", "I", 32, 20),
        "D": ("d", "Q", 64, 20),
    }
    FloatImmeFormat_7x_8x = {
        "H": ("e", "H", 16, 16),
        "F": ("f", "I", 32, 32),
        "D": ("d", "Q", 64, 32),
    }
    ...
    EIATTR_AutoGen_7x_8x = set(
        ["EIATTR_CTAIDZ_USED", "EIATTR_WMMA_USED", "EIATTR_EXIT_INSTR_OFFSETS"]
    )

    EIATTR_AutoGen_5x_6x = set(
        [
            "EIATTR_CTAIDZ_USED",
            "EIATTR_WMMA_USED",
    ...
    def splitIntImmeModifier(self, ins_parser, int_val):
        if (
            self.__mMajor <= 6
            and (not ins_parser.m_InsOp.endswith("32I"))
            and ((int_val & 0x80000) != 0)
        ):
    ...
    def genPredCode(self, ins_info):
        ...
        if s.startswith("U"):
            if s.startswith(
                "UNDEF"
            ):  # UNDEF is reserved for un-disassembled instructions
                return None
            else:
                s2 = "@UP0 " + s
        else:
    ...
    def splitCtrlCodeFromIntList_5x_6x(int_list: list):
        ...
        for i in range(0, len(int_list), 4):
            ...
            c0 += (
                (ccode & CuSMVersion.CCReuse0_5x_6x) >> CuSMVersion.CCPos0_5x_6x
            ) << 64
            c1 += (
                (ccode & CuSMVersion.CCReuse1_5x_6x) >> CuSMVersion.CCPos1_5x_6x
            ) << 64
            c2 += (
                (ccode & CuSMVersion.CCReuse2_5x_6x) >> CuSMVersion.CCPos2_5x_6x
            ) << 64
    ...
    def mergeCtrlCodes_5x_6x(ins_code_list, ctrl_code_list):
        ...
        if len(ctrl_code_list) != n_ins:
            raise Exception(
                "Length of control codes(%d) != length of instruction(%d)!"
        ...
        if nccode_intact * 3 != n_ins:
            ...
            cc, i0, i1, i2 = CuSMVersion.remixCode_5x_6x(
                *t_ins_code_list, *t_ctrl_code_list
            )
            bio.write(cc.to_bytes(8, "little"))
            bio.write(i0.to_bytes(8, "little"))
            bio.write(i1.to_bytes(8, "little"))
